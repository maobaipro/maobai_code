...
def roll_dice(num_rolls, dice=six_sided):
    ...
    sum_score = 0
    pig_out = False
    for _ in range(num_rolls):#通过循环实现多次掷色子
        current_num = dice()
        if current_num == 1:#验证是否发生pig out
           pig_out = True
        sum_score += current_num
    if pig_out:
        return 1
    return sum_score
    # END PROBLEM 1sum_score = 0
...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    pi_remain = pi // (10 ** (100-score))
    # END PROBLEM 2
    return pi_remain % 10 + 3
...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    if player_score == 0 or opponent_score == 0:#特殊情况分开来讨论
        return False
    if player_score > opponent_score:#通过比较大小找出更大的数和跟小的数
        big_nmuber = player_score
        small_number = opponent_score
    else:
        big_nmuber = opponent_score
        small_number = player_score
    while(big_nmuber % small_number != 0):#通过循环求出最大公约数
        temp = small_number
        small_number = big_nmuber % small_number
        big_nmuber = temp
    gcd_number = small_number
    if(gcd_number >= 10):#根据最大公约数的结果进行判断
        return True
    else:
        return False
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if opponent_score - player_score <= 2 and opponent_score > player_score :
        return True
    else:
        return False
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    while score0 < goal and score1 < goal:#首先判断玩家分数是否已经达到目标分数
        if who == 0:#根据玩家的不同，分别使用不同的玩家策略
            dice0 = strategy0(score0,score1)
            score0 += take_turn(dice0,score1,dice)
            if not extra_turn(score0,score1):#判断是否会进行额外的回合
                who = other(who)
        else:#同上
...
def announce_lead_changes(last_leader=None):
    ...
    def say(score0, score1):
        if score0 > score1:#通过分数比较得到目前的领先者
            leader = 0
        elif score1 > score0:
            leader = 1
        else:
            leader = None
        if leader != None and leader != last_leader:#判断是否为新的领先者
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1, 11):
        ...
        for _ in range(trials_count):
            total_score += roll_dice(i,dice)
        average = total_score / trials_count  # 计算平均得分
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if free_bacon(opponent_score) >= cutoff or extra_turn(score+free_bacon(opponent_score),opponent_score)  else num_rolls
    # END PROBLEM 11
